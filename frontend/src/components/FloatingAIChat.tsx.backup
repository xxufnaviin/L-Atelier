...
import { Bot, MessageCircle, X, Send, Sparkles, TrendingUp, BarChart3, Users } from 'lucide-react';
...
export default function FloatingAIChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: "Hi! I'm your AI Trend Analyst. I can help you understand beauty trends, market insights, and campaign strategies. What would you like to know?",
      isUser: false,
      timestamp: new Date()
    }
  ]);
  ...
  const sendMessage = (message: string) => {
    ...
    // Simulate AI response with potential chart
    setTimeout(() => {
      const chartData = generateChartData(message);
      const hasChart = !!chartData;
      
      let response = chatResponses[message as keyof typeof chatResponses];
      
      if (!response) {
        if (hasChart) {
          response = "Here's the visual analysis you requested. The chart below shows the key insights from our trend data analysis.";
        } else {
          response = "I understand you're interested in beauty trends. Let me analyze the current data for you. Based on our latest insights, I recommend focusing on emerging trends like 'Vanilla Girl' aesthetic and 'Glass Skin' techniques, which are showing strong growth in the Malaysian market.";
        }
      }
